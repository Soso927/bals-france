/* =====================================================
   SCRIPT JAVASCRIPT - COFFRET INDUSTRIE
...
/* =====================================================
   ...
   À quoi ça sert ?
   ----------------
   Gère la sélection des cartes cliquables (matériaux).
   ...
   - group = le nom du groupe (pour identifier les cartes liées)
===================================================== */
...
===================================================== */
function selectToggle(label, group) {
...
function updateSummary() {
    ...
    if (distributeur || contactDist || installateur || affaire || email) {
        ...
        // Ajoute la section au tableau des items
        items.push({
            title: 'Contact',
            content: contactContent
        });
        
        // Compte comme 1 champ rempli
        filledFields++;
    }
    
    // ===== SECTION 2 : CARACTÉRISTIQUES =====
    
    // Récupère le matériaux sélectionné
    // :checked = sélecteur CSS pour trouver le radio coché
    const materiau = document.querySelector('input[name="materiau"]:checked');
    
    // Récupère l'IP sélectionné
    const ip = document.querySelector('input[name="ip"]:checked');
    
    // Si au moins un est sélectionné...
    if (materiau || ip) {
        let caracContent = '';
        
        if (materiau) {
            caracContent += `<p>Matériau: ${materiau.value}</p>`;
            filledFields++;
        }
        if (ip) {
            caracContent += `<p>Indice IP: ${ip.value}</p>`;
            filledFields++;
        }
        
        items.push({
            title: 'Caractéristiques',
            content: caracContent
        });
    }
    ...
    let prisesContent = '';
    let hasPrises = false;  // Indique si au moins une prise est ajoutée
    
    // Parcourt chaque champ de quantité
    qtyInputs.forEach(function(input) {
        ...
        if (qty > 0) {
            ...
            // Récupère le nom de la prise (data-name)
            const name = input.getAttribute('data-name');
            
            // Trouve la ligne du tableau contenant cette prise
            const row = input.closest('tr');
            
            // Récupère les listes déroulantes de cette ligne
            const selects = row.querySelectorAll('.detail-select');
            
            // Récupère le brochage (1ère liste) et la tension (2ème liste)
            let brochage = '';
            let tension = '';
            
            if (selects.length >= 2) {
                brochage = selects[0].value || '';
                tension = selects[1].value || '';
            }
            
            // Construit la ligne du résumé
            // Exemple : "2x CEI 16A (3P+T, 400V)"
            let details = '';
            if (brochage || tension) {
                details = ` (${brochage}${brochage && tension ? ', ' : ''}${tension})`;
            }
            
            prisesContent += `<p>${qty}x ${name}${details}</p>`;
        }
    });
    ...
    // Récupère toutes les protections de tête cochées
    const protTete = document.querySelectorAll('input[name="protTete"]:checked');
    
    // Récupère toutes les protections de prises cochées
    ...
    if (protTete.length > 0 || protPrises.length > 0) {
        ...
        if (protTete.length > 0) {
            // Crée une liste des valeurs cochées
            // Array.from() convertit la NodeList en tableau
            // map() transforme chaque élément
            // join() assemble le tableau en une chaîne
            const values = Array.from(protTete).map(cb => cb.value).join(', ');
            protContent += `<p>Protection tête: ${values}</p>`;
            filledFields++;
        }
        
        if (protPrises.length > 0) {
            const values = Array.from(protPrises).map(cb => cb.value).join(', ');
            protContent += `<p>Protection prises: ${values}</p>`;
        }
    ...
    if (observations.trim()) {  // trim() enlève les espaces au début et à la fin
        items.push({
            title: 'Observations',
    ...
    // ===== GÉNÉRATION DU HTML DU RÉSUMÉ =====
    
    if (items.length === 0) {
        // Si aucun item, affiche l'état vide
        ...
        // Génère le HTML pour chaque section
        let html = '';
...
/* =====================================================
   ...
   À quoi ça sert ?
   ----------------
   Efface complètement le formulaire pour recommencer.
===================================================== */
function resetForm() {
...
function generateText() {
    ...
    let text = '=== DEMANDE DE DEVIS - COFFRET INDUSTRIE ===\n\n';
...
function generateMailto() {
    ...
    // Sujet de l'email
    const sujet = `Demande de devis Coffret Industrie - ${affaire}`;
...
/* =====================================================
   ...
   2. selectCard() - Sélectionne une carte (matériaux)
   ...
   7. copierTexte() - Copie dans le presse-papier
   8. generateMailto() - Envoie par email
===================================================== */